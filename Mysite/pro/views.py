...
from .forms import Loginform, RowProduitsForm, RowClientsForm, ClientsForm, \
    OrderForm, Visite_testForm, BcForm
from .models import Clients, Produit, Wilaya, Order, Visite_test, Bc
...
def count_client(request):
    ...
    count_visite_ouest = Allvisite_list.all().filter(région="1").count()
    count_visite_centre = Allvisite_list.all().filter(région="2").count()
    count_visite_est = Allvisite_list.all().filter(région="3").count()
...
def customer(request, pk_test):
    ...
    context = {'customer': customer, 'bc': bc, 'page_obj': page_obj, 'bc_form': bc_form}
...
def visite_list(request):
    ...
    paginator = Paginator(AllVisite_list, 30)  # Show 10 orders per page
